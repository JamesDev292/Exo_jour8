...
asc_array = array.sort { |a, b| a.length <=> b.length }
puts asc_array [0]



puts "Combien y-a-t'il de handle contenant 5 caractères ? (le @ ne compte pas pour un caractère)"
#Sortir le nombre de lettre par handle
nb_letter = array.map {|word| word.length}
#Détecter les handles à 6 lettres 
list = 0
nb_letter.each do |i|
    if i == 6
        list += 1   #Compter les handles à 6 lettres 
    end
end
puts list
...
#Reconnaitre la 2ieme lettre de chaque handle
second_letter = array.map {|mot| mot.slice(1)}
#Déterminer si elle est en majuscule
list = 0
second_letter.count do |i|
    if i =~ /[A-Z]/         #Donner le nombre de handle avec majuscule
        list += 1
    end
end
puts list
...
number_letter = array.map {|word| word.length}
    
counts = Hash.new(0)
number_letter.each { |name| counts[name] += 1 }

counts.each do |key, value|
    puts "Il y a #{value} handle avec #{key} de caractères."
end
